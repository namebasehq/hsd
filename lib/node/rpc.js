...
class RPC extends RPCBase {
  ...
  async getBlockHeader(args, help) {
    ...
    if (!verbose)
      return entry.encode().toString('hex', 0, 80);
  ...
  async _handleWork(data) {
    ...
    const header = Headers.decode(data);

    if (!header.prevBlock.equals(attempt.prevBlock)
    ...
    const nonces = this.merkleMap.get(header.witnessRoot);
  ...
  async _createWork() {
    ...
    const data = attempt.getHeader(root, time, consensus.ZERO_NONCE);
  ...
  async getWork(args, help) {
    if (help || args.length > 1)
      throw new RPCError(errs.MISC_ERROR, 'getwork ( "witnessroot" )');

    if (args.length === 1) {
      const valid = new Validator(args);
      const root = valid.bhash(0);

      if (!root)
        throw new RPCError(errs.TYPE_ERROR, 'Invalid witness root.');

      if (this.merkleMap.has(root))
        return null;
    }
  ...
  async estimateFee(args, help) {
    if (help || args.length < 1 || args.length > 2)
      throw new RPCError(errs.MISC_ERROR, 'estimatefee nblocks (covenantType)');

    const valid = new Validator(args);
    const blocks = valid.u32(0, 1);
    const covenantType = valid.str(1);

    if (!this.fees)
      throw new RPCError(errs.MISC_ERROR, 'Fee estimation not available.');

    const fee = this.fees.estimateFee(blocks, false, covenantType);
