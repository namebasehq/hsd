...
module.exports = {
  ...
  async handleUpdateWithCache(req, res, context) {
    ...
    const { result, fromCache } = await doWithCache(updateCache,
      idempotencyKey,
  ...
  async handleTransferWithCache(req, res, context) {
    ...
    const options = TransactionOptions.fromValidator(valid);
    const { result, fromCache } = await doWithCache(transferCache,
      idempotencyKey,
      () => req.wallet.createTransfer(name, addr, options));
    const mtx = result;
    res.setHeader('From-Cache', fromCache);
  ...
  async handleFinalizeWithCache(req, res, context) {
    ...
    const options = TransactionOptions.fromValidator(valid);
    const { result, fromCache } = await doWithCache(finalizeCache,
      idempotencyKey,
      () => req.wallet.createFinalize(name, options));
    const mtx = result;
    res.setHeader('From-Cache', fromCache);
  ...
  async handleSendMany(req, res, context) {
    ...
    const outputs = filteredSendTo.map((element) => {
      ...
      const addr = util.parseAddress(rawAddress, this.network);
