...
util.retrieveMultipleFromCache = function (cache, keys) {
  const placer = function (cacheHits, retrieved) {
    retrieved.forEach(element => cacheHits.push(element));
  };
  return this.retrieveFromCache(cache, keys, placer);
};
 ...
 *
 * @param {ProcessedOutput} processedOutput transformed output
 * @param {LRU} cache asd
 * @param {function} placer asd
 * @returns {undefined}
 */
...
util.storeMultipleInCache = function (processedOutput, cache) {
  const placer = function (itemMap, item) {
    const idempotencyKey = item.idempotency_key;
    if (itemMap.has(idempotencyKey)) {
      itemMap.get(idempotencyKey).push(item);
    } else {
      itemMap.set(idempotencyKey, [item]);
    }
  };
