...
class RootServer extends DNSServer {
  ...
  async handle(msg, rinfo) {
    ...
    try {
      ...
      if (this.suffix) {
        ...
        if (found) {
          const handshake_name = name.slice(0, -1*this.suffix.length);
          req.question[0].name = handshake_name;
          changed = true;
        }
      }

      let res1 = await this.answer(req, rinfo);
      res = dnssec.stripSignatures(res1)

      if (type === types.SOA && this.soaRewrite === 'true') {
        res.answer[0].data.ns = 'ns1-hsd-zone.namebase.io.';
      }
  ...
  send(req, res, rinfo) {
    ...
    if (!this.dnssec) {
      ...
      console.log(msgWithoutSig)
      const msg = msgWithoutSig.encode()
    }
