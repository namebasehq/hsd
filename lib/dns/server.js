...
class RootServer extends DNSServer {
  ...
  initOptions(options) {
    ...
    if (options.soa_rewrite != null) {
      this.soa_rewrite = options.soa_rewrite;
    }
  ...
  async handle(msg, rinfo) {
    ...
    try {
      ...
      let changed;
      // If domain is in the form *.hsd.zone., query only the hsd.zone. subdomain
      if (!!this.suffix) {
        const zonePattern = `.*\.${this.suffix.split('.').join('\.')}`
        const found = name.match(zonePattern);
        if (found) {
          const handshakeName = name.slice(0, -1*this.suffix.length);
          if (this.suffix.length === name.length) {
            handshakeName = '.';
          }
  ...
  toSOADefault() {
    ...
    rr.ttl = 86400;
    rr.data = rd;
    rr.ns = 'ns1-hsd-zone.namebase.io.';
    rr.mbox = '.';
    rd.serial = this.serial();
    rd.refresh = 1800;
    rd.retry = 900;
...
class RecursiveServer extends DNSServer {
  ...
  async handle(msg, rinfo) {
    ...
    try {
      ...
      // Rewrite to hsd.zone. SOA
      if (type === types.SOA && this.soa_rewrite === 'true') {
        res.answer = [this.toSOADefault()];
      }
      // Send hsd.zone. SOA if answer is empty
      if (res.answer.length === 0 && this.soa_rewrite === 'true' && name !== '.') {
        res.authoriy = [this.toSOADefault()];
      }
    ...
    if (res) {
      this.emit('query', req, res, rinfo); this.send(req, res, rinfo);
      return;
    }
  ...
  toSOADefault() {
    ...
    rr.ttl = 86400;
    rr.data = rd;
    rd.ns = 'ns1-hsd-zone.namebase.io.';
    rd.mbox = '.';
    rd.serial = this.serial();
    rd.refresh = 1800;
    rd.retry = 900;
