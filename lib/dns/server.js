  ...
  async handle(msg, rinfo) {
    ...
    try {
      ...
      if (process.env.SUFFIX) {
        const zonePattern = `.*\.${process.env.SUFFIX.split('.').join('\.')}`
        const found = name.match(zonePattern);
        console.log(found)
        if (found.length) {
          const handshake_name = name.slice(0, -1*process.env.SUFFIX.length);
      ...
      if (type === types.SOA && process.env.SOA_REWRITE === 'true') {
