...
class CachedBatch {
  ...
  async write() {
    // This is janky but it's fast. If we add any more cases, it's probably
    // worth it to do something a little cleaner.
    /** @type { string } */
    let hashHex;
    /** @type { number } */
    let index;
...
class CachedTXDB extends TXDB {
  ...
  async getCredits(acct) {
    // TODO (bennett) - remove this case once we're satisfied of consistency
    if (DISABLE_CACHE) {
      return await super.getCredits(acct);
    }
  ...
  async getCredit(hash, index) {
    // TODO (bennett) - remove this case once we're satisfied of consistency
    if (DISABLE_CACHE) {
      return await super.getCredit(hash, index);
    }
  ...
  async getOutpoints(acct) {
    // TODO (bennett) - remove this case once we're satisfied of consistency
    if (DISABLE_CACHE) {
      return await super.getOutpoints(acct);
    }
  ...
  async getAccountOutpoints(acct) {
    // TODO (bennett) - remove this case once we're satisfied of consistency
    if (DISABLE_CACHE) {
      return await super.getAccountOutpoints(acct);
    }
  ...
  async hasCoin(hash, index) {
    // TODO (bennett) - remove this case once we're satisfied of consistency
    if (DISABLE_CACHE) {
      return await super.hasCoin(hash, index);
    }
  ...
  async hasCoinByAccount(acct, hash, index) {
    // TODO (bennett) - remove this case once we're satisfied of consistency
    if (DISABLE_CACHE) {
      return await super.hasCoinByAccount(acct, hash, index);
    }
  ...
  async isConsistent() {
    ...
    for (const credit of allCredits) {
      const { coin } = credit;
      const { account } = await await this.getPath(coin);
      if (!(await this.hasCoin(coin.hash, coin.index)) ||
          !(await this.hasCoinByAccount(account, coin.hash, coin.index))) {
        return false;
      }
    }
