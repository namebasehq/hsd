...
class TXDB {
  ...
  /**
   * Updates a Bid in levelDB
   * @param {BlindBid} bid bid to update
   * @returns {Promise<void>} void
   */
  async updateBid(bid) {
    const b = this.newBatch();
    this.putBid(b, bid.nameHash, bid.prevout, bid);
    return b.write();
  }
  ...
  async isDoubleOpen(tx) {
    const isDoubleOpenList = await Promise.all(
      tx.outputs.map(({ covenant }) => this.isCovenantDoubleOpen(covenant))
    );
    // any covenant being a double open means
    // the whole transaction includes a double open
   ...
   * @returns {(Promise<Details>|Promise<Details[]>)}
   */
