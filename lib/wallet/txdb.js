...
class TXDB {
  ...
  isLocked(coin, parentEvent) {
    const event = parentEvent
    ? metrics.newEvent('txdb.isLocked', parentEvent)
    : null;

    try {
      const key = event.time('coin.toKey', () => {
        return coin.toKey();
      });
      return event.time('txdb.locked.has', () => {
  ...
  filterLocked(coins, parentEvent) {
    const event = parentEvent
      ? metrics.newEvent('txdb.filterLocked', parentEvent)
      : null;

    if (event) {
      event.add('numCoins', coins.length);
    }
    try {
      const out = [];

      for (const coin of coins) {
        if (!this.isLocked(coin, event))
          out.push(coin);
      }
