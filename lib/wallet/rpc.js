...
class RPC extends RPCBase {
  ...
  init() {
    ...
    this.add('sendopenidempotent', this.sendOpenIdempotent);
    this.add('sendupdateidempotent', this.sendUpdateIdempotent);
  ...
  _validateOpenIdempotent(args, help, method) {
    const msg = `${method} "name" idempotencykey ( force "account" )`;

    if (help || args.length < 2 || args.length > 4)
      throw new RPCError(errs.MISC_ERROR, msg);

    const valid = new Validator(args);
    const name = valid.str(0);
    const idempotencyKey = valid.str(1);
    const force = valid.bool(2, false);
    const account = valid.str(3);

    if (!name || !rules.verifyName(name))
      throw new RPCError(errs.TYPE_ERROR, 'Invalid name.');

    return { name, force, idempotencyKey, account };
  }
  ...
  _validateBidIdempotent(args, help, method) {
