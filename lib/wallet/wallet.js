...
class Wallet extends EventEmitter {
  ...
  async _createClaim(name, options) {
    ...
    if (proof) {
      ...
      const zone = zones.length >= 2 ? zones[zones.length - 1] : null;
    ...
    if (minFee == null) minFee = policy.getMinFee(vsize, rate);
  ...
  async makeFakeClaim(name, options) {
    ...
    if (!proof) throw new Error('Could not resolve name.');

    proof.addData([txt]);

    const data = proof.getData(this.network);

    if (!data) throw new Error(`No valid DNS commitment found for ${name}.`);
  ...
  async makeBatchOpen(names, force, acct) {
    ...
    for (const name of names) {
      ...
      if (!rules.hasRollout(nameHash, height, network)) {
        errorMessages.push({
          name: name,
          error: `Name not yet available: "${name}".`,
        });
        continue;
      }
  ...
  async _createBatchOpen(names, force, options) {
    ...
    const { mtx, errors, isAllError } = await this.makeBatchOpen(
      names,
      force,
      acct
    );
  ...
  async _createOpen(name, force, options) {
    const { mtx, errors, isAllError } = await this._createBatchOpen(
      Array.of(name),
      force,
      options
    );
  ...
  async makeBid(name, value, lockup, acct) {
    const { mtx } = await this.makeBatchBid([{ name, value, lockup }], acct);
    return mtx;
  }
  ...
  async makeBatchBid(bids, acct) {
    ...
    for (const bid of bids) {
      try {
        ...
        const output = await this.processBid(
          name,
          value,
          lockup,
          acct,
  ...
  async processBid(name, value, lockup, acct, consecutiveCall) {
    ...
    if (!rules.verifyName(name)) throw new Error(`Invalid name: "${name}".`);
    ...
    if (!ns) ns = await this.wdb.getNameStatus(nameHash);
    ...
    if (value > lockup) throw new Error(`Bid exceeds lockup value: "${name}".`);

    const addr = !consecutiveCall
      ? await this.receiveAddress(acct)
  ...
  async _createAuctionTxs(name, value, lockup, options) {
    const { mtx: bid } = await this._createBid(
      [{ name, value, lockup }],
      options
    );
  ...
  async processNameForReveal(domainName, acct) {
    ...
    if (!ns) throw new Error(`Auction not found: "${domainName}".`);
    ...
    for (const { prevout, own } of bids) {
      ...
      // Is local?
      if (coin.height < ns.height) continue;

      const blind = coin.covenant.getHash(3);
      const bv = await this.getBlind(blind);

      if (!bv) throw new Error(`Blind value not found: "${domainName}".`);
  ...
  async makeBatchReveal(names, acct) {
    ...
    const { validDomains, rejectedDomains } = util.createStrictBatch(
      MAX_REVEALS_PER_BATCH_TX,
      outputMap
    );
    const mtx = new MTX();
  ...
  async makeRevealAll() {
    ...
    for (const { nameHash, prevout, own } of bids) {
      ...
      if (state < states.REVEAL) continue;
    ...
    if (mtx.outputs.length === 0) throw new Error('No bids to reveal.');
  ...
  async makeRedeem(name, acct) {
    ...
    if (!ns) throw new Error(`Auction not found: "${name}".`);
    ...
    for (const { name, data: resource } of names) {
      try {
        ...
        if (targetLength > MAX_FINISH_COUNT) {
          throw new Error(
            `${name} output length ${finishesLength}` +
  ...
  async makeRedeemAll() {
    ...
    for (const { nameHash, prevout, own } of reveals) {
      ...
      if (ns.isExpired(height, network)) continue;

      const state = ns.state(height, network);

      if (state < states.CLOSED) continue;
    ...
    if (mtx.outputs.length === 0) throw new Error('No reveals to redeem.');
  ...
  async _makeRegister(name, resource) {
    ...
    if (!coin) throw new Error(`Wallet did not win the auction: "${name}".`);
  ...
  async _sendTransfer(name, address, options) {
    const passphrase = options ? options.passphrase : null;
    const mtx = await this._createTransfer(name, address, options);
    return this.sendMTX(mtx, passphrase);
  }
  ...
  async finalize(mtx, options) {
    ...
    assert(
      mtx.verifyInputs(this.wdb.height + 1, this.network),
      'TX failed context check.'
    );
    assert(
      this.wdb.height + 1 >= this.network.txStart,
  ...
  async sendMTX(mtx, passphrase) {
    ...
    for (const output of tx.outputs) {
      ...
      if (output.value > 0) {
        if (!output.address) throw new Error('Cannot send to unknown address.');
  ...
  async increaseFee(hash, rate, passphrase) {
    ...
    for (const input of mtx.inputs) input.witness.clear();
  ...
  async resend() {
    ...
    for (const wtx of wtxs) {
      if (!wtx.tx.isCoinbase()) txs.push(wtx.tx);
    }
  ...
  async getWalletCoinView(tx, view) {
    if (!(view instanceof CoinView)) view = new CoinView();

    if (!tx.hasCoins(view)) view = await this.txdb.getCoinView(tx);
    ...
    for (const input of tx.inputs) {
      ...
      // Unharden the account index, if necessary.
      if (path.account & HD.common.HARDENED) path.account ^= HD.common.HARDENED;
