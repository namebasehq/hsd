...
class Wallet extends EventEmitter {
  ...
  getReceive(acct = 0, index = 0) {
    return this.getKey(acct, index, 0);
  }
  ...
  getChange(acct = 0, index = 0) {
    return this.getKey(acct, index, 1);
  }
  ...
  async getKey(acct, index, branch) {
    const unlock = await this.writeLock.lock();
    try {
      return await this._getKey(acct, index, branch);
    } finally {
      unlock();
    }
  }
  ...
  async _getKey(acct, index, branch) {
    const account = await this.getAccount(acct);

    if (!account)
      throw new Error('Account not found.');

    return account.getKey(index, branch);
  }
