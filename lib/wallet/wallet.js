...
class Wallet extends EventEmitter {
  ...
  async makeBatchOpen(names, force, acct) {
    ...
    for (const name of names) {
      ...
      // TODO: Handle expired behavior.
      if (rules.isReserved(nameHash, height, network)) {
        errorMessages.push({ name: name, error: 'Name is reserved' });
        continue;
      }
  ...
  /**
   * abandonAll abandons all pending state txes
   * for this wallet, if no acct is passed it
   * abandons all txes within wallet
   *
   * @param {(Number|String)?} acct wallet account
   * @returns {Promise<Array<PromiseSettledResult>>} array of settled result
   */
  async abandonAll(acct = -1) {
    const pendingTxHashes = await this.txdb.getPendingHashes(acct);
    const unlock = await this.writeLock.lock();
    try {
      const abandonedTxPromises = [];
      pendingTxHashes
        .map(txHash => this._abandon(txHash))
        .forEach(abandonTxPromise => abandonedTxPromises.push(abandonTxPromise));
      return Promise.allSettled(abandonedTxPromises);
    } finally {
      unlock();
    }
  }
