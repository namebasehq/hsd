...
/** @typedef { import('../utils/metrics').Event } Event */
...
class Wallet extends EventEmitter {
  ...
  async fill(mtx, options, parentEvent) {
    const event = metrics.newEvent('wallet.fill', parentEvent || this.honey);
    try {
      if (!options)
        options = {};

      const acct = options.account || 0;
      const change = await event.asyncSubEvent(
        'wallet.changeAddress',
        () => this.changeAddress(acct)
      );
      ...
      if (rate == null) {
        event.startTimer('estimateFee');
        rate = await this.wdb.estimateFee(options.blocks);
        event.stopTimer('estimateFee');
      }

      let coins;

      if (options.smart) {
        event.startTimer('getSmartCoins');
        coins = await this.getSmartCoins(options.account);
        event.stopTimer('getSmartCoins');
      } else {
        coins = await this.getCoins(options.account, event);
        coins = this.txdb.filterLocked(coins, event);
      }

      event.startTimer('mtx.fund');
  ...
  /**
   * Make a open MTX.
   * @param {String} name the name to open
   * @param {Boolean} force force?
   * @param {Number|String} acct the account to open from
   * @param {Event} parentEvent a metrics tracing event
   * @returns {MTX} a mutable open transaction
   */
  async makeOpen(name, force, acct, parentEvent) {
    const event = metrics.newEvent(
      'wallet.makeOpen', parentEvent || this.honey
    );
    ...
    try {
      ...
      event.addField('error', err.message);
      throw err;
    } finally {
      event.send();
    }
  ...
  async _createOpen(name, force, options, event) {
    const acct = options ? options.account || 0 : 0;
    const mtx = await this.makeOpen(name, force, acct, event);
    await this.fill(mtx, options, event);
    return this.finalize(mtx, options, event);
  }
  ...
  async createOpen(name, force, options, parentEvent) {
    const event = metrics.newEvent(
      'wallet.createOpen', parentEvent || this.honey
    );
    event.addField('hnsName', name);
    const unlock = await event.measureLockWait(this.fundLock);
    try {
      return await this._createOpen(name, force, options, event);
    } finally {
      unlock();
      event.send();
    }
  ...
  async _sendOpen(name, force, options, event) {
    const passphrase = options ? options.passphrase : null;
    const mtx = await this._createOpen(name, force, options, event);
    event.startTimer('sendMTX');
    const result = this.sendMTX(mtx, passphrase, event);
    event.stopTimer('sendMTX');
    return result;
  }
  ...
  async sendOpen(name, force, options, parentEvent) {
    const event = metrics.newEvent(
      'wallet.sendOpen', parentEvent || this.honey
    );
    event.addField('hnsName', name);
    const unlock = await event.measureLockWait(this.fundLock);
    try {
      return await this._sendOpen(name, force, options, event);
    } finally {
      event.time('unlockWait', () => unlock());
      // unlock();
      event.send();
    }
  }
  ...
  async sendOpenIdempotent(name, force, idempotencyKey, options, parentEvent) {
    const event = metrics.newEvent(
      'wallet.sendOpenIdempotent', parentEvent || this.honey
    );
    event.addField('hnsName', name);
    const unlock = await event.measureLockWait(this.fundLock);
    try {
      if (idempotencyKey && this.sendOpenResults.has(idempotencyKey)) {
        const result = this.sendOpenResults.get(idempotencyKey);
        event.addField('fromCache', true);
        return [result, true];
      }

      const result = await this._sendOpen(name, force, options, event);
      if (idempotencyKey) {
        this.sendOpenResults.set(idempotencyKey, result);
      }
      event.addField('fromCache', false);
      return [result, false];
    } finally {
      unlock();
      event.send();
  ...
  async makeBid(name, value, lockup, acct, parentEvent) {
    const event = metrics.newEvent(
      'wallet.makeBid', parentEvent || this.honey
    );
    try {
      assert(typeof name === 'string');
  ...
  async _createBid(name, value, lockup, options, event) {
    const acct = options ? options.account || 0 : 0;
    const mtx = await this.makeBid(name, value, lockup, acct, event);
    await this.fill(mtx, options, event);
    return this.finalize(mtx, options, event);
  }
  ...
  async createBid(name, value, lockup, options, parentEvent) {
    const event = metrics.newEvent(
      'wallet.createBid', parentEvent || this.honey
    );
    event.addField('hnsName', name);
    const unlock = await event.measureLockWait(this.fundLock);
    try {
      return await this._createBid(name, value, lockup, options, event);
    } finally {
      unlock();
      event.send();
    }
  ...
  async _sendBid(name, value, lockup, options, event) {
    const passphrase = options ? options.passphrase : null;
    const mtx = await this._createBid(name, value, lockup, options, event);
    event.startTimer('sendMTX');
    const result = this.sendMTX(mtx, passphrase);
    event.stopTimer('sendMTX');
    return result;
  }
  ...
  async sendBid(name, value, lockup, options, parentEvent) {
    const event = metrics.newEvent(
      'wallet.sendBid', parentEvent || this.honey
    );
    event.addField('hnsName', name);
    const unlock = await event.measureLockWait(this.fundLock);
    try {
      ...
      const result = await this._sendBid(name, value, lockup, options, event);
      if (idempotencyKey) {
  ...
  async makeReveal(name, parentEvent) {
    assert(typeof name === 'string');
    const event = metrics.newEvent(
      'wallet.makeReveal',
      parentEvent || this.honey
    );
    ...
    try {
      ...
      for (const {prevout, own} of bids) {
        ...
        const coin = await this.getCoin(hash, index, event);
  ...
  async _createReveal(name, options, event) {
    const mtx = await this.makeReveal(name, event);
    await this.fill(mtx, options, event);
    return this.finalize(mtx, options, event);
  }
  ...
  async createReveal(name, options, parentEvent) {
    const event = metrics.newEvent(
      'wallet.createReveal',
      parentEvent || this.honey
    );
    ...
    try {
      return await this._createReveal(name, options, event);
    } finally {
      unlock();
      event.send();
    }
  ...
  async _sendReveal(name, options, event) {
    const passphrase = options ? options.passphrase : null;
    const mtx = await this._createReveal(name, options, event);
    event.startTimer('sendMTX');
    const result = this.sendMTX(mtx, passphrase);
    event.stopTimer('sendMTX');
    return result;
  ...
  async sendReveal(name, options, parentEvent) {
    const event = metrics.newEvent(
      'wallet.sendReveal',
      parentEvent || this.honey
    );
    ...
    try {
      return await this._sendReveal(name, options, event);
    } finally {
      unlock();
      event.send();
    }
  ...
  async makeRedeem(name, parentEvent) {
    const event = metrics.newEvent(
      'wallet.makeRedeem', parentEvent || this.honey
    );
  ...
  async _createRedeem(name, options, event) {
    const mtx = await this.makeRedeem(name, event);
    await this.fill(mtx, options, event);
    return this.finalize(mtx, options, event);
  }
  ...
  async createRedeem(name, options, parentEvent) {
    const event = metrics.newEvent(
      'wallet.createRedeem', parentEvent || this.honey
    );
    event.addField('hnsName', name);
    const unlock = await event.measureLockWait(this.fundLock);
    try {
      return await this._createRedeem(name, options, event);
    } finally {
      unlock();
      event.send();
    }
  ...
  async _sendRedeem(name, options, event) {
    const passphrase = options ? options.passphrase : null;
    const mtx = await this._createRedeem(name, options, event);
    event.startTimer('sendMTX');
    const result = this.sendMTX(mtx, passphrase);
    event.stopTimer('sendMTX');
    return result;
  ...
  async sendRedeem(name, options, parentEvent) {
    const event = metrics.newEvent(
      'wallet.sendRedeem', parentEvent || this.honey
    );
    event.addField('hnsName', name);
    const unlock = await event.measureLockWait(this.fundLock);
    try {
      return await this._sendRedeem(name, options, event);
    } finally {
      unlock();
      event.send();
    }
  ...
  async _makeRegister(name, resource, parentEvent) {
    const event = metrics.newEvent(
      'wallet._makeRegister',
      parentEvent || this.honey
    );
  ...
  async _makeRawRegister(name, resourceHex, parentEvent) {
    const event = metrics.newEvent(
      'wallet._makeRawRegister',
      parentEvent || this.honey
    );
    try {
      assert(typeof name === 'string');
  ...
  async makeUpdate(name, resource, parentEvent) {
    const event = metrics.newEvent(
      'wallet.makeUpdate', parentEvent || this.honey
    );
    try {
      assert(typeof name === 'string');
      ...
      if (coin.covenant.isReveal() || coin.covenant.isClaim())
        return this._makeRegister(name, resource, event);
  ...
  async makeRawUpdate(name, resourceHex, parentEvent) {
    const event = metrics.newEvent(
      'wallet.makeRawUpdate', parentEvent || this.honey
    );
    try {
      assert(typeof name === 'string');
      ...
      if (coin.covenant.isReveal() || coin.covenant.isClaim())
        return this._makeRawRegister(name, resourceHex, event);
  ...
  async _createUpdate(name, resource, options, event) {
    const mtx = await this.makeUpdate(name, resource, event);
    await this.fill(mtx, options, event);
    return this.finalize(mtx, options, event);
  }
  ...
  async _createRawUpdate(name, resourceHex, options, event) {
    const mtx = await this.makeRawUpdate(name, resourceHex, event);
    await this.fill(mtx, options, event);
    return this.finalize(mtx, options, event);
  }
  ...
  async createUpdate(name, resource, options, parentEvent) {
    const event = metrics.newEvent(
      'wallet.createUpdate', parentEvent || this.honey
    );
    event.addField('hnsName', name);
    const unlock = await event.measureLockWait(this.fundLock);
    try {
      return await this._createUpdate(name, resource, options, event);
    } finally {
      unlock();
      event.send();
    }
  ...
  async createRawUpdate(name, resourceHex, options, parentEvent) {
    const event = metrics.newEvent(
      'wallet.createRawUpdate', parentEvent || this.honey
    );
    const unlock = await event.measureLockWait(this.fundLock);
    try {
      return await this._createRawUpdate(name, resourceHex, options, event);
    } finally {
      unlock();
      event.send();
    }
  ...
  async _sendUpdate(name, resource, options, event) {
    const passphrase = options ? options.passphrase : null;
    const mtx = await this._createUpdate(name, resource, options, event);
    event.startTimer('sendMTX');
    const result = this.sendMTX(mtx, passphrase);
    event.stopTimer('sendMTX');
    return result;
  ...
  async _sendRawUpdate(name, resourceHex, options, event) {
    const passphrase = options ? options.passphrase : null;
    const mtx = await this._createRawUpdate(name, resourceHex, options, event);
    event.startTimer('sendMTX');
    const result = this.sendMTX(mtx, passphrase);
    event.stopTimer('sendMTX');
    return result;
  ...
  async sendUpdate(name, resource, options, parentEvent) {
    const event = metrics.newEvent(
      'wallet.sendUpdate', parentEvent || this.honey
    );
    event.addField('hnsName', name);
    const unlock = await event.measureLockWait(this.fundLock);
    try {
      const idempotencyKey = options ? options.idempotencyKey : null;
      if (idempotencyKey) {
        if (this.sendUpdateResults.has(idempotencyKey)) {
          event.addField('fromCache', true);
          return this.sendUpdateResults.get(idempotencyKey);
        }
      }

      const result = await this._sendUpdate(name, resource, options, event);
      if (idempotencyKey) {
        event.addField('fromCache', false);
        this.sendUpdateResults.set(idempotencyKey, result);
      }
      return result;
    } finally {
      unlock();
  ...
  async sendRawUpdate(name, resourceHex, options, parentEvent) {
    const event = metrics.newEvent(
      'wallet.sendRawUpdate', parentEvent || this.honey
    );
    event.addField('hnsName', name);
    const unlock = await event.measureLockWait(this.fundLock);
    try {
  ...
  async finalize(mtx, options, parentEvent) {
    const event = metrics.newEvent(
      'wallet.finalize',
      parentEvent || this.honey
    );
    try {
      ...
      if (options.paths === true) {
        event.startTimer('getWalletCoinView');
        mtx.view = await this.getWalletCoinView(mtx, mtx.view);
        event.stopTimer('getWalletCoinView');
      }

      event.startTimer('template');
      const total = await this.template(mtx);
      event.stopTimer('template');
  ...
  async sendMTX(mtx, passphrase, parentEvent) {
    const event = metrics.newEvent(
      'wallet.sendMTX', parentEvent || this.honey
    );

    try {
      await this.sign(mtx, passphrase, event);

      if (!mtx.isSigned())
        throw new Error('TX could not be fully signed.');

      const tx = event.time('mtx.toTX', () => {
        return mtx.toTX();
      });

      // Policy sanity checks.
      event.startTimer('sanityChecks');
      ...
      event.stopTimer('sanityChecks');

      await event.asyncSubEvent('wdb.addTX', () => this.wdb.addTX(tx));
      this.logger.debug('Sending wallet tx (%s): %x', this.id, tx.hash());

      event.startTimer('wdb.send');
      await this.wdb.send(tx);
      event.stopTimer('wdb.send');
  ...
  async sign(mtx, passphrase, parentEvent) {
    const event = metrics.newEvent(
      'wallet.sign', parentEvent || this.honey
    );
    try {
      if (this.watchOnly)
  ...
  getCoin(hash, index, parentEvent) {
    const event = metrics.newEvent('wallet.getCoin', parentEvent || this.honey);
    try {
  ...
  async getCoins(acct, parentEvent) {
    const event = metrics.newEvent(
      'wallet.getCoins', parentEvent || this.honey
    );
    try {
      const account = await this.ensureIndex(acct);

      return event.time('txdb.getCoins', () => {
