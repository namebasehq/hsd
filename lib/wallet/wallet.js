...
class Wallet extends EventEmitter {
  ...
  async fill(mtx, options = {}) {
    ...
    const selected = await this.select(mtx, {
      ...
      remapSelection: options.remapSelection,
  ...
  async select(mtx, options) {
    const remap = options.remapSelection ?? true;
    let selection = options.selection || DEFAULT_SELECTION;

    if (remap) {
      switch (selection) {
        case 'all':
          options.selection = common.coinSelectionTypes.DB_ALL;
          selection = options.selection;
          break;
        case 'value':
          options.selection = common.coinSelectionTypes.DB_VALUE;
          selection = options.selection;
          break;
        case 'age':
          options.selection = common.coinSelectionTypes.DB_AGE;
          selection = options.selection;
          break;
        case 'sweepdust':
          options.selection = common.coinSelectionTypes.DB_SWEEPDUST;
          selection = options.selection;
          break;
      }
    }
  ...
  async _sendOpen(name, options) {
    ...
    return this.sendMTX(mtx, passphrase, options);
  }
  ...
  async makeFinish(name, rawResource, acct, mtx) {
    assert(typeof name === 'string');
    ...
    for (const {prevout, own} of reveals) {
      ...
      if (prevout.equals(ns.owner)) {
        ...
        // REGISTER pays second highest.
        output.value = ns.value;
        output.covenant.setRegister(
          nameHash, ns.height, rawResource, blockHash);
      } else {
        // REDEEM gets full value back.
        output.value = coin.value;
        output.covenant.setRedeem(nameHash, ns.height);
      }
  ...
  */
  ...
  async makeBatch(actions, options = {}) {
    ...
    const handleAction = async (action, args) => {
      switch (action.type) {
        ...
        case 'FINISH': {
          assert(args.length === 2, 'Bad arguments for FINISH');
          const name = args[0];
          const rawResource = args[1];
          await this.makeFinish(name, rawResource, acct, mtx);
          break;
        }
        case 'RENEW': {
          if (args.length === 1) {
  ...
  async _sendBatch(actions, options) {
    ...
    return {
      mtx,
      tx: mtx ? await this.sendMTX(mtx, passphrase, options) : null,
      errors,
      outputMap
    };
  }
  ...
  async sendMTX(mtx, passphrase, options) {
...
class WalletCoinSource extends AbstractCoinSource {
  ...
  async next() {
    ...
    if (this.signal && this.signal.aborted) {
      ...
      const error = new common.AbortError(
        'Coin selection aborted.',
