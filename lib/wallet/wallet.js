...
class Wallet extends EventEmitter {
  ...
  async _createBatchOpen(names, force, options) {
    ...
    const {
      ...
      errors,
      isAllError
    } = await this.makeBatchOpen(names, force, acct);
  ...
  async _sendBatchOpen(names, force, options) {
    const passphrase = options ? options.passphrase : null;
    const {
      mtx,
      errors,
      isAllError
    } = await this._createBatchOpen(names, force, options);
  ...
  async sendBatchOpen(names, force, options) {
    const unlock = await this.fundLock.lock();
    try {
      return await this._sendBatchOpen(names, force, options);
    } finally {
      unlock();
    }
  }
  ...
  async _createOpen(name, force, options) {
    const {
      ...
      errors,
      isAllError
    } = await this._createBatchOpen(Array.of(name), force, options);
    if (isAllError) {
  ...
  */
  ...
  async makeBatchRenewal(names, acct) {
    ...
    for (const name of names) {
      try {
        ...
        errorMessages.push({ name: name, errorMessage: error.message ? error.message : error.toString() });
      }
    }
  ...
  async _createBatchRenewal(names, options) {
    const acct = options ? options.account : null;
    const { mtx: mtxToFillAndFinalize, errors } = await this.makeBatchRenewal(names, acct);
