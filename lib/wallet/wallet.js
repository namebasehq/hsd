...
class Wallet extends EventEmitter {
  ...
  constructor(wdb, options) {
    ...
    const updatesPerAuction = 3; // average in March 2020 is ~3.1
    const updateCapacity =
      consensus.MAX_BLOCK_OPENS * updatesPerAuction * updatesCacheSizeInBlocks;
    this.sendUpdateResults = new LRU(capacity);
    ...
    try {
      ...
      if (idempotencyKey) {
        if (this.sendUpdateResults.has(idempotencyKey)) {
          return this._sendUpdate(name, value, lockup, options);
        }
      }

      const result = await this._sendUpdate(name, value, lockup, options);
      if (idempotencyKey) {
  ...
  async sendRawUpdate(name, resourceHex, options) {
    console.log('SEND RAW UPDATE');
    const unlock = await this.fundLock.lock();
    try {
      const idempotencyKey = options ? options.idempotencyKey : null;
      if (idempotencyKey) {
        if (this.sendRawUpdateResults.has(idempotencyKey)) {
          return this._sendRawUpdate(name, value, lockup, options);
        }
      }

      const result = await this._sendRawUpdate(name, value, lockup, options);
      if (idempotencyKey) {
        this.sendRawUpdateResults.set(idempotencyKey, result);
      }
      return result;
    } finally {
      unlock();
