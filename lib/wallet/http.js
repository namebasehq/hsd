...
class HTTP extends Server {
  ...
  initRouter() {
    ...
    this.get('/wallet/:id/balance/details', async (req, res) => {
      /** @type {Wallet} */
      ...
      for (const account of accounts) {
        ...
        for (const coin of coins) {
          ...
          if (isLockedCovenant(coin.covenant)) {
            totalLockedValue += coin.value;
            countMap = lockedCounts;
          } else {
            totalValue += coin.value;
          }
          const covenantName = rules.typesByVal[coin.covenant.type];
          countMap[covenantName] = (countMap[covenantName] || 0) + 1;
        }

        resultMap[`${wallet.id}/${account}`] = {
          totalValue,
          counts,
          totalLockedValue,
          lockedCounts
