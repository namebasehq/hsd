  ...
  initRouter() {
    ...
    this.post('/wallet/:id/batch/bid', async (req, res) => {
      ...
      assert(bids.length > 0 && bids.length <= MAX_BIDS_ARRAY_LENGTH, 
        `Bids are required to be non empty and shoud not exceed ${MAX_BIDS_ARRAY_LENGTH}`);
      
      // ensure all bids are valid, reject whole batch if any invalid
      // TODO with new hsd-proxy cache - remove BidCache from here
      bids.forEach((element) => {
        ...
        if (!validator.has('name') || 
          ...
          throw new Error('Bids array contains a Bid with missing mandatory fields!')
        } 
        else { // check types
          validator.str('name');
          validator.u64('bid');
     ...
     const bids2 = bids.map(element => {
        return {
          name: element.name,
          value: element.bid,
          lockup: element.lockup,
          idempotencyKey: element.idempotencyKey
        };
      })

      // TODO below properties are undocumented ?
      // And wont be present in batch bid request at all 
      const options = TransactionOptions.fromValidator(valid);

      // if already exists in the cache do not process it
      let results = [];
      let errors  = [];

      const filteredBids = bids2.filter(bid => {
          const bidResultFromCache = req.wallet.sendBatchBidResults.get(bid.idempotencyKey);
          if (bidResultFromCache != null) {
              // mark this result is from cache
              bidResultFromCache["fromCache"] = true;
              results.push(bidResultFromCache);
          }
          return (bidResultFromCache == null);
      });

      if (filteredBids.length > 0 ) {
        const {mtx, errorMessages} = await req.wallet.createBatchBid(filteredBids, options);
        ...
        const processedBids = util.postProcessBatchBids(txJSON, mtx);

        // update cache
        processedBids.forEach(element => {
          req.wallet.sendBatchBidResults.set(element.idempotency_key, element);     
        });

        results = [...results, ...processedBids];
      }

      return res.json(200, {processedBids: results, errorMessages: errors});
