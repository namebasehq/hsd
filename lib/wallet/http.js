...
class HTTP extends Server {
  ...
  initRouter() {
    ...
    this.get('/wdb', async (req, res) => {
      if (!req.admin) {
        res.json(403);
        return;
      }

      const state = this.wdb.state;

      res.json(200, {
        general: {
          version: this.wdb.version,
          state: {
            startHeight: state.startHeight,
            startHash: state.startHash.toString('hex'),
            height: state.height,
            marked: state.marked
          }
        },
        guards: {
          confirming: this.wdb.confirming,
          rescanning: this.wdb.rescanning,
          filterSent: this.wdb.filterSent
        },
        locks: {
          readLock: this.wdb.readLock.busy,
          writeLock: this.wdb.writeLock.busy,
          txLock: this.wdb.txLock.busy
        }
      });
    });
    ...
    // Wallet Live check
    this.get('/wallet/:id/checklock', async (req, res) => {
      const unlock = await req.wallet.fundLock.lock();
      unlock();
      res.json(200, { success: true });
    });
...
class TransactionOptions {
  ...
  constructor(valid, network, signal) {
    ...
    this.abortOnClose = false;
    this.controller = null;
    this.timeout = null;
    this.outputs = [];

    if (valid)
      this.fromValidator(valid, network, signal);
  }
  ...
  fromValidator(valid, network, controller) {
    assert(valid);
    ...
    if (valid.has('abortOnClose'))
      this.abortOnClose = valid.bool('abortOnClose');

    if (valid.has('timeout')) {
      assert(controller);
      this.abortOnClose = this.abortOnClose || valid.has('timeout');
      this.timeout = valid.float('timeout') * 1000 | 0;
    }
  ...
  static fromValidator(valid, network, controller) {
    return new this(valid, network, controller);
  }
}
...
function TXCreateTimeout(options) {
  if (!options.timeout || !options.controller)
    return null;

  return setTimeout(() => {
    if (!options.controller.signal.aborted)
      options.controller.abort(new Error('Timed out.'));
  }, options.timeout);
}
...
HTTP.TXCreateTimeout = TXCreateTimeout;
