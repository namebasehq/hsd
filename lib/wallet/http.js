    ...
    this.post('/wallet/:id/batch/bid', async (req, res) => {
      ...
      bids.forEach((element) => {
        ...
        if (!validator.has('name') || 
          ...
          throw new Error('Bids array contains a Bid with missing madatory fields!')
        } 
        else { // check types
          validator.str('name');
          validator.u64('bid');
     ...
     const bids2 = bids.map(element => {
      ...
      // Create Batch Bids here
      const {mtx, errorMessages} = await req.wallet.createBatchBid(bids2, options)
