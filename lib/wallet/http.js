...
class HTTP extends Server {
  ...
  initRouter() {
    ...
    this.del('/wallet/:id/tx/:hash', async (req, res) => {
      ...
      const hashes = (await req.wallet.abandon(hash))
        .map(hash => hash.toString('hex'));
      res.json(200, { success: true, txs: hashes });
    });

    // Abandon all Pending Txes
    this.del('/wallet/:id/tx', async (req, res) => {
      const abandonedTxes = await req.wallet.abandonAll(-1);
      res.json(200, { success: true, abandonedTxes });
    });
    ...
    this.post('/wallet/:id/bid', async (req, res) => {
      ...
      if (broadcast) {
        const isBlockingSend = (bid <= 10_000_000);
        let tx;
        if (isBlockingSend) {
          tx = await req.wallet.sendMTXBlocking(mtx, passphrase);
        } else {
          tx = await req.wallet.sendMTX(mtx, passphrase);
        }
   ...
   * functionality
   *
   * @param {*} httpMethod string value of one of
   * 'post', 'get', 'put', 'patch', 'delete'
   * @param {*} path location handler is listening
   * @param {*} handler http handler that will handle requests
