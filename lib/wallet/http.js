  ...
  initRouter() {
    ...
    this.post('/wallet/:id/batch/finish', async (req, res) => {
      ...
      names.forEach((element) => {
        ...
        if (!dataValidator.has('records'))
           throw new Error('data must be in form: {records: []}');

        validator.array(element.data.records);
      });

      names = names.map(({name, data}) => {
      ...
      names.forEach(({name, data}) => {
        const cachedNames = finishCache.get(name);
        if (!cachedNames) {
          uniqueNames.push({name, data});
        } else {
          cachedNames.forEach(cachedName => processedFinishes.push(cachedName));
        }
      });
      ...
      if (uniqueNames.length > 0) {
        ...
        // always broadcast
        const {mtx, errorMessages: errors} = await req.wallet
           .createBatchFinish(uniqueNames, options);

        errorMessages = errors;
        const tx = await req.wallet.sendMTX(mtx, passphrase);
        const txJSON = tx.toJSON(this.network);

        const outputsLength = txJSON.outputs.length;
        for (let i=0; i<outputsLength; i++) {
          const output = txJSON.outputs[i];
          const covenant = output.covenant;
          const covenantType = covenant.type;

          if (covenantType === rules.types.REDEEM ||
              covenantType === rules.types.REGISTER) {
      ...
      return res.json(200, {processedFinishes,errorMessages});
    });
