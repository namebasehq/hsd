...
class HTTP extends Server {
  ...
  initRouter() {
    ...
    this.post('/wallet/:id/batch/finish', async (req, res) => {
      const valid = Validator.fromRequest(req);
      let finishRequests = valid.array('finishRequests');
      const passphrase = valid.str('passphrase');
      ...
      finishRequests = finishRequests.map(({name, data}) => {
      ...
      finishRequests.forEach(({name, data}) => {
      ...
      if (uniqueNames.length > 0) {
        ...
        // always broadcast
        let mtx;
        ({mtx, errorMessages} = await req.wallet
        ...
        util.postProcessBatchFinishes(
          txJSON, finishCache, nameHashMap, processedFinishes);
      }
