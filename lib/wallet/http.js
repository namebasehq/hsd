  ...
  initRouter() {
    ...
    this.post('/wallet/:id/batch/bid', async (req, res) => {
     ...
     bids = bids.map((element) => {
      ...
      const unsentBids = bids.filter((bid) => {
          const bidResultFromCache = req.wallet
          .sendBatchBidResults.get(bid.idempotencyKey);
      ...
      if (unsentBids.length > 0) {
        const {mtx, errorMessages} = await req.wallet
          .createBatchBid(unsentBids, options);
