...
class WalletDB extends EventEmitter {
  ...
  async close() {
    ...
    this.abortWalletControllers(new Error('Closing walletdb.'));
  ...
  async _addBlock(entry, txs) {
    ...
    if (tip.height === this.state.height) {
      ...
      this.abortWalletControllers('Starting a rescan');
    ...
    this.abortWalletControllers('Received a new block.');
  ...
  abortWalletControllers(message) {
    ...
    for (const wallet of this.wallets.values())
      wallet.abortAllControllers(error);
  }
}
