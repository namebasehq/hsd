...
class WalletDB extends EventEmitter {
  ...
  async scan(height) {
    ...
    const plannedScanCount = this.state.height - height + 1;
    this.logger.info('WalletDB is scanning %d blocks.', plannedScanCount);
    ...
    try {
      ...
      this.emit('rescanFinished', { plannedScanCount });
    } finally {
      this.rescanning = false;
    }
  }
  ...
  async _deepClean() {
    ...
    const message = 'Deep Clean complete. A rescan is now required.';
    this.emit('deepCleanFinished', { message });
  ...
  async send(tx, isSendBlocking) {
    return this.client.send(tx, isSendBlocking);
  }
