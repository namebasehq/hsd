...
const PATCHED_LOOKAHEAD = 100000;
...
class Account extends bio.Struct {
  ...
  constructor(wdb, options) {
    ...
    this.receiveDepth = 0;
    this.changeDepth = 0;
    this.lookahead = PATCHED_LOOKAHEAD;
  ...
  async resyncDepth(b, receiveDepth, changeDepth) {
    let derived = false;

    if (receiveDepth < this.receiveDepth) {
      const depth = this.receiveDepth + this.lookahead;

      for (let i = receiveDepth; i < depth; i++) {
        const key = this.deriveReceive(i);
        await this.saveKey(b, key);
      }
    ...
    if (changeDepth < this.changeDepth) {
      ...
      for (let i = changeDepth; i < depth; i++) {
        const key = this.deriveChange(i);
        await this.saveKey(b, key);
      }
  ...
  changeAddress() {
    let key;
    if (this.staticAddress) {
      // even though this is change, we still want to use the same address
      key = this.getSpecificKey(0, 0);
    } else {
      key = this.changeKey();
    }
  ...
  format() {
    ...
    return {
      ...
      staticAddress: this.staticAddress,
  ...
  read(br) {
    ...
    this.staticAddress = (flags & 0x80) !== 0;
    this.type = br.readU8();
    this.m = br.readU8();
    this.n = br.readU8();
